...
CREATE TABLE IF NOT EXISTS `dbEtablissement`.`COMPTES` (
  ...
  `id_enseignant` INT NULL,
  `id_administrateur` INT NULL,
  `id_eleve` INT NULL,
  `email_verification` INT NULL,
  PRIMARY KEY (`id_compte`),
  INDEX `fk_COMPTES_ENSEIGNANTS1_idx` (`id_enseignant` ASC) ,
  INDEX `fk_COMPTES_ADMINISTRATEURS1_idx` (`id_administrateur` ASC) ,
  INDEX `fk_COMPTES_ELEVES1_idx` (`id_eleve` ASC) ,
  CONSTRAINT `fk_COMPTES_ENSEIGNANTS1`
    FOREIGN KEY (`id_enseignant`)
    REFERENCES `dbEtablissement`.`ENSEIGNANTS` (`id_enseignant`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_COMPTES_ADMINISTRATEURS1`
    FOREIGN KEY (`id_administrateur`)
    REFERENCES `dbEtablissement`.`ADMINISTRATEURS` (`id_administrateur`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_COMPTES_ELEVES1`
